...
class JVMProfiler:
    ...
    def getProfileData(self,tag,program,benchmark,interval,samples):
	...
	#time.sleep(2)
        ...
        p = subprocess.Popen('ps -ef | grep java > vmstat.txt', stdout=subprocess.PIPE, shell=True)
        p_status = p.wait()
        f = open ('vmstat.txt','r')
        lines = f.readlines()
        f.close()
	...
	#time.sleep(5)
        for line in lines:
            if ((str(benchmark) or str(':XX') in line) and ('python' not in line) and ('/bin/sh' not in line)):
                ...
                for option in optionset:
                    ...
                    p = subprocess.Popen(jstat_command + ' > jstat_'+str(benchmark)+'_'+str(option)+'.txt', stdout=subprocess.PIPE, shell=True)
        ...
        for option in optionset:  
            file = open('jstat_'+str(benchmark)+'_'+str(option)+'.txt','r')
            ...
            statfile = open ('jstat_'+str(benchmark)+'_'+str(option)+'.csv','w')
            statfile.write("\n".join(stat_lines))
            
    def getStatistics(self,program,benchmark):
        ...
        for option in optionset:
            #print option
            dataframe_dict[option] = pd.read_csv('jstat_'+str(benchmark)+'_'+str(option)+'.csv',usecols=coldict[option])
	...
	for option in optionset:
            #print option
            dataframe_dict[option].to_csv('jstat_'+str(benchmark)+'_'+str(option)+'.csv',index=False)	
...
if __name__ == '__main__':
    ...
    profiler.getProfileData('dacapo',args.RunCommand,args.ProgramName,100,'')
    profiler.getStatistics(args.RunCommand,args.ProgramName)
